...
- **The sentence that introduces the grade scale** carries every grade word in backticks, in order, cheapest first, and carries no other backticked word. The parser finds that sentence by a form of *grade* standing in it — `grade`, `grades`, `graded`, `grading`, `grau`, `gradua…` — so that word belongs there too, and **no `.`, `!` or `?` with whitespace after it may fall inside it**: the section is cut into sentences at every such mark before the scale is looked for, so one inner full stop, an abbreviation included, leaves the grade word in one piece and the scale words in the next — and where the cut falls inside the scale itself the half holding the grade word still parses, so a vocabulary missing its top words reads as a healthy one.
...
Declining is an answer. A project that does not want the discipline writes no section, and nothing below ever fires — not a gap, and not something to argue anyone out of. To see which it is:
    ...
    python3 ~/.claude/plugins/marketplaces/mcardia-claude-kit/plugins/operator-decision-gate/hooks/check-od-record.py --explain .

It names the constitution the walk resolved and the vocabulary parsed out of it, or reports the project ungated and lists the filenames it looked for. That path is the marketplace clone, which carries no version segment and so can be pasted as it stands; the installed copy of the same file sits under `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/`, printed in full as this plugin's `installPath` in `~/.claude/plugins/installed_plugins.json`.
...
**Order it for a decision, not for a review.** The reader opens the block to answer something, not to audit a chain of reasoning. So: what is being decided, then what the reader would observe, then why it stands that way, then the recommendation. Reasoning supports the recommendation; it never precedes the question. **Cost, effort and risk travel with the thing they price** — inside each way forward where there are several, inside the recommendation where there is one — because a recommendation whose price is unstated cannot be answered, only agreed with, and agreement is not the thing being asked for. The remedy lens produces that sizing; this is where it lands.

**Options go inline, in the block.** Never a question widget, never a popup, never a separate round trip that costs a turn. Where there are two or three ways to go, each gets its real shape and its price — the screen the reader would see, the command they would run, the files that move — never an abstract label. "Option A: refactor" is not an option, it is a category.

**A stack, tooling or library choice is scored, not asserted** — a table inside the recommendation, one row per candidate, and the winner marked. Where the project has a convention of its own for scoring a choice of this kind, the columns are that convention's rather than ones invented for this decision, and the block ends on the trade-off the winner buys, never on a bare verdict.
...
>
> **\<label: the recommendation\>** — KEEP. Raising the number is a one-constant change that moves the failure to a longer outage without removing it; what removes it is the per-endpoint backoff `docs/adr-019.md:22` already schedules. Keeping costs nothing in this repository and leaves the next incident identical to this one until that ADR ships.
>
