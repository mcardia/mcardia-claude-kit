...
**The operator-decision section this interview can write is read by another plugin.**
`/sdd-generators:constitution` has an operator-decisions stage that collects the whole
rule — what counts as one, the gate, the grade scale and the line, the categories
reserved whatever the grade, the record's fields, and where a record lands — and an
`AGENTS.md` template that renders it. This plugin writes that section and never reads it.
What applies and enforces it is `operator-decision-gate`, a separate install in this same
marketplace, and that plugin never writes the section: two authors for one fact is the
drift this kit exists to prevent. The stage is skippable and the interview talks nobody
out of skipping it — a project that declines gets no section, and a project that keeps it
without installing the gate has a rule that binds whoever reads it and nothing that
refuses a violation.
...
The plugin registers one subagent. It is the authority on what its own lenses mean — the
skills name a lens and never restate it, so there is one definition to keep true. It pins
its effort in its definition and does not pin a model, because what a fleet of agents
costs is the operator's call, not the plugin's.
