...
In the form the constitution states it: verify the CAUSE and the REMEDY at source, both, and by something other than the pass that produced them. Then grade the change with one word — `low`, `minor`, `moderate`, `major`, `critical` — weighing the change **whole**: what it costs to make, and how far it reaches. At `moderate` or below the coordinating session executes and reports it as done; above `moderate` it is the operator's. A short list of categories is the operator's **at every grade** — the constitution enumerates them; read them there rather than from this file, because a stale copy here would be precisely the drift the kit exists to prevent.
...
- **The cheapest-tier ladder, in writing, all four**: (1) accept it, because a decision already taken prices this risk — name the decision; (2) product or UX makes the case unreachable; (3) one line in something that already exists; (4) a new mechanism. A one-liner that leaves the cause standing — a widened bound, a longer timeout — is NOT tier 3: it is an accept with a number on it, priced at tier 1.
